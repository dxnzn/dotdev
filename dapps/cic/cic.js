(() => {
  ...
  function fmtMoney(n) {
    return fmtMoneyCore(n, 2, false) || `$${n.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
  }
  ...
  function calculate(state) {
    ...
    let yearContrib = 0, yearInterest = 0;
  ...
  function renderOverview(container, state, result) {
    ...
    const compLabel = freqLabel[state.compoundFreq] || `${state.compoundFreq}\xD7/yr`;
    ...
    if (state.contribution > 0) {
      ...
      contribLine = "No recurring contributions are set up \u2014 growth comes entirely from compounding your initial deposit.";
    }
    let earnLine = "";
    ...
    el.innerHTML = `
      ...
      <p style="margin-top:0.75rem">Of your final balance, <code>${fmtMoney(result.totalInterest)}</code> is pure interest \u2014 money that your money made <i>for</i> you. That means your total deposits of <code>${fmtMoney(totalIn)}</code> grew by <code>${growthPct}%</code>, a <code>${multiplier}\xD7</code> multiple on what you put in.</p>
  ...
  function renderFormula(container, state) {
    ...
    let zeroNote = "";
    if (state.rate === 0) {
      zeroNote = '<p style="margin-top:0.75rem;color:var(--amber)">With a 0% rate, contributions grow linearly: A = P + C \xD7 n \xD7 t (no compounding).</p>';
    }
    f.innerHTML = `
      <p>Compound interest grows your principal by applying earned interest back onto itself each compounding period.</p>
      <div class="formula-block">A = P(1 + r/n)<sup>nt</sup> + C \xD7 [((1 + r/n)<sup>nt</sup> \u2212 1) / (r/n)]</div>
      ...
      <p style="margin-top:0.75rem">If an annual contribution increase <code>g</code> is specified, each year's contribution becomes <code>C \xD7 (1 + g)<sup>year\u22121</sup></code>, escalating deposits over time.</p>
  ...
  function renderTable(container, data) {
    ...
    if (!tableBody) return;
    tableBody.innerHTML = data.yearlyData.map(
      (d) => `<tr><td>${d.yearLabel}</td><td>${fmtMoneyWhole(d.deposits)}</td><td>${fmtMoneyWhole(d.interest)}</td><td>${fmtMoneyWhole(d.cumulativeInterest)}</td><td>${fmtMoneyWhole(d.balance)}</td></tr>`
    ).join("");
  }
  ...
  function updateReport(container, _state, result) {
    ...
    if (reportBody) {
      reportBody.innerHTML = result.yearlyData.map(
        (d) => `<tr><td>${d.yearLabel}</td><td>${fmtMoneyWhole(d.deposits)}</td><td>${fmtMoneyWhole(d.interest)}</td><td>${fmtMoneyWhole(d.cumulativeInterest)}</td><td>${fmtMoneyWhole(d.balance)}</td></tr>`
      ).join("");
    }
  ...
  function init(container, isReport) {
    ...
    const commaIntFields = /* @__PURE__ */ new Set(["principal", "contribution"]);
    ...
    function syncContribSlider() {
      ...
      contribSlider.min = 0;
      contribSlider.max = isMonthly ? 1e4 : 1e5;
